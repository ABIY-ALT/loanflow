...
import { ArrowLeft, FolderKanban, ExternalLink, Loader2, AlertCircle, Building, Flame, History } from 'lucide-react';
...
export default function DepartmentQueuePage() {
      ...
      try {
        ...
        if (loansResult.error) {
          ...
          setUnassignedLoans(filteredLoans);
           ...
           setError(`No loan data received for unassigned queue.`);
           setUnassignedLoans([]);
        }
      } catch (err: any) {
  ...
  if (error && unassignedLoans.length === 0) {
    return (
