          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              <div className="grid md:grid-cols-2 gap-8">
                <FormField control={form.control} name="customerName" render={({ field }) => ( <FormItem> <FormLabel>Customer Name</FormLabel> <div className="relative"> <UserIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /> <FormControl> <Input placeholder="e.g., John Doe" {...field} className="pl-10" disabled={isSubmitting} /> </FormControl> </div> <FormMessage /> </FormItem> )} />
                <FormField control={form.control} name="customerEmail" render={({ field }) => ( <FormItem> <FormLabel>Customer Email</FormLabel> <div className="relative"> <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /> <FormControl> <Input type="email" placeholder="e.g., john.doe@example.com" {...field} className="pl-10" disabled={isSubmitting} /> </FormControl> </div> <FormMessage /> </FormItem> )} />
                ...
                <FormField control={form.control} name="customerBranch" render={({ field }) => ( <FormItem> <FormLabel>Customer Branch</FormLabel> <div className="relative"> <Briefcase className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground z-10" /> <FormControl> <Select onValueChange={field.onChange} defaultValue={field.value} disabled={isSubmitting} > <SelectTrigger className="pl-10"> <SelectValue placeholder="Select customer branch" /> </SelectTrigger> <SelectContent> {MOCK_BRANCHES.map(branch => ( <SelectItem key={branch} value={branch}>{branch}</SelectItem> ))} </SelectContent> </Select> </FormControl> </div> <FormMessage /> </FormItem> )} />
                ...
                <FormField control={form.control} name="loanType" render={({ field }) => ( <FormItem> <FormLabel>Loan Type</FormLabel> <div className="relative"> <ListFilter className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground z-10" /> <FormControl> <Select onValueChange={field.onChange} defaultValue={field.value} disabled={isLoadingLoanTypes || isSubmitting || availableLoanTypes.length === 0} > <SelectTrigger className="pl-10"> <SelectValue placeholder={isLoadingLoanTypes ? "Loading loan types..." : "Select loan type"} /> </SelectTrigger> <SelectContent> {availableLoanTypes.map(type => ( <SelectItem key={type} value={type}>{type}</SelectItem> ))} {availableLoanTypes.length === 0 && !isLoadingLoanTypes && ( <SelectItem value="no-types-found-disabled" disabled>No loan types with workflows found</SelectItem> )} </SelectContent> </Select> </FormControl> </div> {loanTypesError && <FormMessage>{loanTypesError}</FormMessage>} {!loanTypesError && availableLoanTypes.length === 0 && !isLoadingLoanTypes && ( <p className="text-sm text-muted-foreground">No loan types with active workflows are configured. Please contact an admin.</p> )} <FormMessage /> </FormItem> )} />
              </div>
