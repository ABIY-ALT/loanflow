    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Assign this task to staff members in the <span className="font-semibold">{currentDepartment || 'N/A'}</span> department.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 py-4">
            ...
            <DialogFooter className="pt-4">
              ...
              <Button type="submit" disabled={isSaving || !canAssignStaff}> {isSaving && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Save Assignment </Button>
            </DialogFooter>
          </form>
        </Form>
      </DialogContent>
